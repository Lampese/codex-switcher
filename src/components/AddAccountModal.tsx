...
export function AddAccountModal({
  ...
  const handleOAuthLogin = async () => {
    if (!name.trim()) {
      setError(t("modal.addAccount.error.enterName"));
      return;
    }
  ...
  const handleImportFile = async () => {
    ...
    if (!fileSource) {
      setError(t("modal.addAccount.error.selectAuthFile"));
      return;
    }
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl w-full max-w-md mx-4 shadow-xl">
        ...
        <div className="flex border-b border-gray-100 dark:border-gray-800">
          {(["oauth", "import"] as Tab[]).map((tab) => (
            ...
            >
              {tab === "oauth"
                ? t("modal.addAccount.tab.oauth")
                : t("modal.addAccount.tab.import")}
            </button>
          ))}
        </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              {t("modal.addAccount.field.accountName")}
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder={t("modal.addAccount.field.accountNamePlaceholder")}
              ...
              {oauthPending ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-gray-700 dark:text-gray-300 font-medium mb-2">
                    {t("modal.addAccount.oauth.waiting")}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">
                    {t("modal.addAccount.oauth.openLink")}
                  </p>
                    ...
                    <button
                      onClick={() => {
                        void navigator.clipboard
                          ...
                          .catch(() => {
                            setError(t("modal.addAccount.error.clipboard"));
                          });
                      }}
                  ...
                  {!tauriRuntime && (
                    <p className="text-xs text-amber-600">
                      {t("modal.addAccount.oauth.sameHost")}
                    </p>
                  )}
                </div>
              ) : (
                <p>
                  {t("modal.addAccount.oauth.generateHint")}
                </p>
              )}
            </div>
          )}
          ...
          {activeTab === "import" && (
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                {t("modal.addAccount.import.selectFile")}
              </label>
              <div className="flex gap-2">
                <div className="flex-1 px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-600 dark:text-gray-300 truncate">
                  {describeFileSource(
                    fileSource,
                    t("modal.addAccount.import.noFileSelected")
                  )}
                </div>
                <button
                ...
                >
                  {t("modal.addAccount.import.browse")}
                </button>
              </div>
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">
                {t("modal.addAccount.import.hint")}
              </p>
            </div>
          )}
        ...
        <div className="flex gap-3 p-5 border-t border-gray-100 dark:border-gray-800">
          ...
          >
            {t("modal.addAccount.button.cancel")}
          </button>
          <button
          ...
          >
            {loading
              ? t("modal.addAccount.button.adding")
              : activeTab === "oauth"
                ? t("modal.addAccount.button.generateLogin")
                : t("modal.addAccount.button.import")}
          </button>
        </div>
      </div>
    </div>
