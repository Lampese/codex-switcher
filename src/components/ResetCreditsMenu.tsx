...
function formatExpiryDetail(
  ...
  if (expiry === "No expiry") return t("credits.no.expiry");
  if (expiry === "Expiry unavailable") return t("credits.expiry.unavailable");
  return t("credits.expires", { date: expiry });
}
...
export function ResetCreditsMenu({
  ...
  const countLabel = t(count === 1 ? "credits.one.reset" : "credits.count.resets", { count });
  ...
  const nextExpiryLabel =
    nextExpiry === "No expiry"
      ...
      : nextExpiry === "Expiry unavailable"
        ? t("credits.expiry.unavailable")
        : t("credits.closest", { date: nextExpiry });
  ...
  return (
    <div ref={wrapperRef} className="relative min-w-0 max-w-full">
      <button
        ...
        title={t("credits.button.title", { count: countLabel, expiry: nextExpiryLabel })}
      >
        <span
          className={
            compact
      ...
      {isOpen && (
        <div
          id={popupId}
          role="dialog"
          aria-label={t("credits.reset.expiry.details")}
              ...
              >
                ...
                <div className="min-w-0">
                  <div className="truncate font-medium text-gray-800 dark:text-gray-200">
                    {credit.title?.trim() || t("credits.reset.number", { count: index + 1 })}
                  </div>
                  <div className="mt-0.5 text-[11px] text-gray-500 dark:text-gray-400">
                    {formatExpiryDetail(credit.expires_at, t)}
                  </div>
                </div>
              </div>
            ))}
