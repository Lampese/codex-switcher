    ...
    async (accountId: string, accountName: string) => {
      ...
      try {
        ...
        showWarmupToast(
          t(
            window.kind === "session"
              ? "warmup.autoSessionSentFor"
              : "warmup.autoWeeklySentFor",
            { name: accountName }
          )
        );
      } catch (err) {
            ...
            {hasNoMatchingAccounts && (
              <div className="rounded-2xl border border-dashed border-gray-300 px-6 py-12 text-center dark:border-gray-700">
                <h2 className="text-base font-semibold text-gray-900 dark:text-gray-100">
                  {t("accounts.noMatchingTitle")}
                </h2>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  {t("accounts.noMatchingBody")}
                </p>
              </div>
            )}
              ...
              <section>
                <div className="flex items-center justify-between gap-3 mb-4">
                  <h2 className="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                    {t("accounts.otherHeading", {
                      count: normalizedAccountSearchQuery
